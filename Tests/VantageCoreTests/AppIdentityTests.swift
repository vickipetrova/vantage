...
///
/// Found in the wild: a row called `20251209_headshot_ai` with no icon, sitting beside the real
/// "HeadshotAI" it was part of. Names below are invented but the shape is the real one.
final class AppIdentityTests: XCTestCase {
    ...
    func testAPhantomKeyedBySKUMergesIntoItsApp() {
        let days = [
            ...
            day(19, [app("photo_ai_sku", "photo_ai_sku", units: 0, usd: 30)]),
            // The day before: the app itself sold, so this report carries the SKU mapping.
            day(18, [app("6756327585", "HeadshotAI", sku: "photo_ai_sku", units: 4, usd: 12)]),
        ]
        ...
        XCTAssertEqual(yesterday?.apps.first?.appleID, "6756327585")
        XCTAssertEqual(yesterday?.apps.first?.title, "HeadshotAI",
    ...
    func testAPhantomAndTheRealAppOnTheSameDayAreSummedNotReplaced() {
        let days = [day(19, [
            app("6756327585", "HeadshotAI", sku: "photo_ai_sku", units: 4, usd: 12),
            app("photo_ai_sku", "photo_ai_sku", units: 0, usd: 30),
        ])]
        ...
        XCTAssertEqual(apps?.first?.title, "HeadshotAI")
    }
    ...
    func testAPhantomWithNoMappingAnywhereIsLeftAsItIs() {
        let days = [day(19, [app("photo_ai_sku", "photo_ai_sku", units: 0, usd: 30)])]
        let apps = AppIdentity.resolve(days).first?.apps
        XCTAssertEqual(apps?.count, 1)
        XCTAssertEqual(apps?.first?.appleID, "photo_ai_sku")
    }

    func testOrdinaryDaysArePassedThroughUnchanged() {
        let days = [day(19, [app("6756327585", "HeadshotAI", sku: "photo_ai_sku"),
                             app("6748345644", "FitGrid", sku: "fitgrid_sku")])]
    ...
    func testDaysWithNoSKUsAtAllAreUntouched() {
        let days = [day(19, [app("6756327585", "HeadshotAI"), app("photo_ai_sku", "photo_ai_sku")])]
        XCTAssertEqual(AppIdentity.resolve(days), days)
    }
    ...
    func testTheIndexPrefersTheMostRecentTitle() {
        let days = [day(19, [app("6756327585", "HeadshotAI Pro", sku: "photo_ai_sku")]),
                    day(18, [app("6756327585", "HeadshotAI", sku: "photo_ai_sku")])]
        XCTAssertEqual(AppIdentity.skuIndex(days)["photo_ai_sku"]?.title, "HeadshotAI Pro")
    }
    ...
    func testAPhantomIsNotTreatedAsASourceOfTruth() {
        let days = [day(19, [app("photo_ai_sku", "photo_ai_sku", sku: "photo_ai_sku")])]
    ...
    func testDayLevelTotalsAreNotDisturbed() {
        let original = DaySales(
            ...
            apps: [app("6756327585", "HeadshotAI", sku: "photo_ai_sku"),
                   app("photo_ai_sku", "photo_ai_sku")],
